function b_struct = initialize_barrier_model_Cape_Hatteras
%#ok<*NASGU>
%which modules to run
name = 'BRIE Cape Hatteras'; 
barrier_model_on = true;
ast_model_on = true;
inlet_model_on = true;
buffer_model_on = true;
calibration_on = true;
real_waves = true;
make_gif = false;
plot_on = false;
...
%feed shorelines to BRIE
Initial_shoreline = 'BRIE_initial_shoreline_Cape_Hatteras.csv'; %initial shoreline
Current_shoreline  = 'Current_shoreline_Cape_Hatteras.csv'; %current shoreline/ shoreline at end calibration period

%general parameters
rho_w = 1025; %density water kg/m3
g = 9.81;

%wave climate parameters derrived from wavehindcast/forecast
wave_height = 1.27; %m Transport-equivalent significant wave height.
wave_period = 7.34; %s Alongshore-transport-weighted mean spectral period (Tm02)
shoreface_moment = 3931.21 ;%  mean(Hs^5 * Tm02^2.5) calculated from a wave hindcast/forecast
wave_pdf = 'wave_pdf_by_island_orientation.csv'; % a transport-weighted sampling distribution PDF 

wave_asym = 0.4; %if real waves  are turned off
wave_high = 0.1;

%barrier model parameters
slr = 5e-3; %sea level rise m/yr
s_background = 6.3333e-4; %background slope (beta)
w_b_crit = 716.572704481389; %critical barrier width m
h_b_crit = 16.55063406616; %critical barrier height m
Qow_max = 49.9607160199077; %max overwash flux m3/m/yr

z = 0; %initial sea level (m)
bb_depth = 3; %back barrier depth (m)

grain_size = 2e-4; %(m)
R = 1.65; %relative density of sand

% e_s = 0.1; %shoreface efficiency values from Aleja/Ashton
% c_s = 0.1;

phi = 1.00691985249466e-05;

%alongshore stretch
dy = 50; %length of each alongshore section (m)
ny = 3004; %number of alonghsore sections
if buffer_model_on
    ny_buffer = round(ny*0.15); %buffer on each side is x percent of ny
else
    %Do nothing (only use ny)
end

%timestepping
dt = 0.05; %years
nt = 3.4e3; %number of timesteps
dtsave = 20; %save spacing
