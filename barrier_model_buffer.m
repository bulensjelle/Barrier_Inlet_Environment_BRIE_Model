function [b_out] = barrier_model_buffer(b_struct)


v2struct(b_struct); %retrieve all model parameters back from structure
drowned = false; %barrier starts alive

%dependent variables
init_xt = 19e3; %km distance of shoreface toe
...
d_sf= 20; %estimated depth in Duck
...
k_sf = (3600*24*365)./(d_sf-z0).*(g^(15/4)*phi*shoreface_moment/(1024*pi^(5/2)*w_s^2) *(4/11*(1/z0^(11/4)-1/(d_sf^(11/4)))));

%s_sf_eq = 3*w_s/4/sqrt(d_sf*g)*(5+3*wave_period^2*g/4/(pi^2)/d_sf); %equilibrium shoreface slope
s_sf_eq = d_sf./init_xt;

% --- Load external wave PDF from CSV (overrides synthetic if present)
if real_waves
    csvfile = wave_pdf; %change to file location
    T = readtable(csvfile);
    % angle column
    if ismember('angle_rad', T.Properties.VariableNames)
        AngArray = T.angle_rad(:);
    elseif ismember('angle_deg', T.Properties.VariableNames)
        AngArray = deg2rad(T.angle_deg(:));
    else
        error('CSV must contain angle_rad or angle_deg column.');
    end
    % pdf column (accept common names)
    if ismember('wave_pdf', T.Properties.VariableNames)
        wave_pdf = T.wave_pdf(:);
    elseif ismember('wave_pdf_mean_barrier', T.Properties.VariableNames)
        wave_pdf = T.wave_pdf_mean_barrier(:);
    else
        error('CSV must contain wave_pdf column.');
    end
    % optional cdf
    if ismember('wave_cdf', T.Properties.VariableNames)
        wave_cdf = T.wave_cdf(:);
    else
        wave_cdf = cumsum(wave_pdf);
    end
    % sanitize & normalize
    wave_pdf = max(0, wave_pdf(:));
    if sum(wave_pdf) <= 0, error('Loaded wave_pdf sums to zero or negative.'); end
    wave_pdf = wave_pdf ./ sum(wave_pdf);
    wave_cdf = cumsum(wave_pdf);
    AngArray = AngArray(:);
    % ensure AngArray in radians
    if max(abs(AngArray)) > pi
        AngArray = deg2rad(AngArray);
    end
    wave_climl = numel(wave_pdf);
else
    % fallback to original synthetic PDF (unchanged)
    ...
    wave_pdf = 4*[wave_asym*wave_high*ones(wave_climl/4,1);wave_asym*(1-wave_high)*ones(wave_climl/4,1);...
         (1-wave_asym)*(1-wave_high)*ones(wave_climl/4,1);(1-wave_asym)*wave_high*ones(wave_climl/4,1)]./wave_climl;
    AngArray = linspace(-0.5*pi,0.5*pi,wave_climl)'; % radians
    %warning('Using  wave synthetic wave PDF');
end
...
di = [(ny+2*ny_buffer),2:(ny+2*ny_buffer),1:(ny+2*ny_buffer),1:(ny+2*ny_buffer)-1,1];
dj = [1,1:(ny+2*ny_buffer)-1,1:(ny+2*ny_buffer),2:(ny+2*ny_buffer),(ny+2*ny_buffer)];

%initial conditions
%Load CoastSat initial shoreline

BRIE_init = readtable(Initial_shoreline);
if buffer_model_on
    
    x_sb1 = -BRIE_init.x_s(1)*ones(ny_buffer,1); %making the buffer 1 as 1th shoreline position
    x_sb2 = -BRIE_init.x_s(ny)*ones(ny_buffer,1); %making the buffer 2 as last shoreline position
    x_so = -BRIE_init.x_s;
    x_s = cat(1,x_sb1,x_so,x_sb2); % new x_s has buffers
    x_t = x_s - 19000*ones(ny+2*ny_buffer,1); %initiating toe for buffered x_s
    x_b = x_s + w_b_crit*ones(ny+2*ny_buffer,1); %same for barrier
    h_b = h_b_crit+zeros(ny+2*ny_buffer,1); %and height barrier m
    barrier_volume = [];
    inlet_idx_close_mat = [];
    inlet_idx = {};
    inlet_idx_mat = [];
    inlet_y = zeros(ny+2*ny_buffer,1);

    y = 0:dy:dy*(ny_buffer + ny-1 + ny_buffer); %alongshore array with buffers on each side
    
    c_idx = zeros(ny+2*ny_buffer,1000,'uint8');

else
    ny_buffer = 0; %there are no buffers

    x_t = -BRIE_init.x_s - 19000*ones(ny,1);
    %x_t = (z-d_sf)./s_background + zeros(ny,1); %position shoreface toe m
    x_s = -BRIE_init.x_s;
    %x_s = rand(ny,1)+d_sf/s_sf_eq+x_t; %position shoreline m
    x_b = -BRIE_init.x_s + w_b_crit*ones(ny,1);
    %x_b = d_sf/s_sf_eq+w_b_crit+x_t; %position back barrier m
    h_b = h_b_crit+zeros(ny,1); %height barrier m
    barrier_volume = [];
    inlet_idx_close_mat = [];
    inlet_idx = {};
    inlet_idx_mat = [];
    ...
    y = 0:dy:(dy*(ny-1)); %alongshore array without buffers
...
x_t_save = zeros(ny,length(1:dtsave:nt),'int32'); x_t_save(:,1) = x_t(ny_buffer+1:ny_buffer+ny);
x_s_save = zeros(ny,length(1:dtsave:nt),'int32'); x_s_save(:,1) = x_s(ny_buffer+1:ny_buffer+ny);
x_b_save = zeros(ny,length(1:dtsave:nt),'int32'); x_b_save(:,1) = x_b(ny_buffer+1:ny_buffer+ny);
h_b_save = zeros(ny,length(1:dtsave:nt),'single'); h_b_save(:,1) = h_b(ny_buffer+1:ny_buffer+ny);
...
if plot_on,
    fig = figure('color','white','Name',name,'visible','on',...
        'InvertHardCopy', 'off');
    ax = gca;
    set(ax,'fontsize',15,'Layer','top','XLim',[0 ny*dy/1000],'Ylim',[-40000 10000],'box','on',...
...
for i=2:length(t) % years
    ...
    if sum(w<-10)>((ny+2*ny_buffer)/2) || any(w<-1000)
    ...
    if inlet_model_on
        ...
        x_b_fld_dt = zeros((ny+2*ny_buffer),1);
        ...
        if mod(t(i),10)<(dt/2) && numel(inlet_idx)<inlet_max,
            ...
            if isempty([inlet_idx{:}]),
                basin_length = Jmin+zeros((ny+2*ny_buffer),1);
            else,
                %change%
               basin_length = min(min(Jmin,2* dy*abs(bsxfun(@minus,1:(ny+2*ny_buffer),reshape(bsxfun(@plus,[-(ny+2*ny_buffer) 0 (ny+2*ny_buffer)],[inlet_idx{:}]'),[],1)))))';
            end
        ...
        if ~isempty(inlet_idx)
            ...
            inlet_idx_mat(inlet_all_idx_idx(diff([inlet_all_idx(end)-(ny+2*ny_buffer) inlet_all_idx])<=1)) = [];
        ...
        if ~isempty(inlet_idx),
            ...
            [inlet_all_idx,inlet_all_idx_idx] = sort(inlet_idx_mat);
            inlet_dist = diff([inlet_all_idx(end)-(ny+2*ny_buffer) inlet_all_idx inlet_all_idx(1)+(ny+2*ny_buffer)]);
            ...
            %we don't have to think about this one ever again!
        ...
        for j=1:length(inlet_idx), %inlet morphodynamics per inlet
            ...
            if inlet_idx{j}==new_inlet,
                new_inlet_idx = mod(new_inlet+(1:wi_cell(j))-2,(ny+2*ny_buffer))+1;
            ...
            inlet_idx(j) = {mod(inlet_idx{j}(1)+(1:wi_cell(j))-2,(ny+2*ny_buffer))+1};
            inlet_nex(j) = {mod(inlet_idx{j}(end),(ny+2*ny_buffer))+1};
            inlet_prv(j) = {mod(inlet_idx{j}(1)-2,(ny+2*ny_buffer))+1};
            ...
            inlet_idx{j} = mod(inlet_idx{j} + migr_int - 1,(ny+2*ny_buffer)) + 1;
    ...
    if ast_model_on,
        ...
        RHS = x_s + r_ipl.*(x_s([2:(ny+2*ny_buffer),1]) - 2*x_s + x_s([(ny+2*ny_buffer),1:(ny+2*ny_buffer)-1])) + x_s_dt;
    ...
    if sedstrat_on && max(x_b) > bar_strat_x && min(x_s) < bar_strat_x %barrier recording ON
        ...
        if isempty(inlet_idx),
            ...
            idx_inl = zeros(size(1,(ny+2*ny_buffer)));
            for j=1:length(inlet_idx),
                idx_inl = idx_inl | (ismembc(1:(ny+2*ny_buffer),inlet_idx{j}) & ((x_s(inlet_nex{j})+w_b_crit)>bar_strat_x)  & ((x_s(inlet_nex{j}))<bar_strat_x));
            end
        end
    ...
    if mod(i,dtsave)==1,
        if buffer_model_on
            %only save the real coastline (not the buffers)
            x_t_save(:,1+fix(i/dtsave)) = x_t(ny_buffer+1:ny_buffer+ny);
            x_s_save(:,1+fix(i/dtsave)) = x_s(ny_buffer+1:ny_buffer+ny);
            x_b_save(:,1+fix(i/dtsave)) = x_b(ny_buffer+1:ny_buffer+ny);
            h_b_save(:,1+fix(i/dtsave)) = h_b(ny_buffer+1:ny_buffer+ny);
            s_sf_save(:,1+fix(i/dtsave)) = s_sf(ny_buffer+1:ny_buffer+ny);
...
% For calibration: Minimal stats + RMSE for shoreline change
if calibration_on
    % Calibration files (begin and end time periode coastsat segments):
    fileInit = Initial_shoreline; %initial shoreline
    fileNow  = Current_shoreline; %current shoreline/ shoreline at end calibration period

    % Read observed tables and compute observed change
    Tinit = readtable(fileInit); Tinit.id = string(Tinit.id);
    Tnow  = readtable(fileNow);  Tnow.id  = string(Tnow.id);
    Tobs = innerjoin(Tinit, Tnow, 'Keys','id', ...
        'LeftVariables',{'id','x_s'}, 'RightVariables',{'x_s'});
    Tobs.Properties.VariableNames = {'id','x_init','x_now'};
    Tobs.dx = Tobs.x_now - Tobs.x_init;

    % Model change: require b_out.x_s_save in workspace (as original script)
    x_s_save_cal = double(b_out.x_s_save);
    nsave_cal = size(x_s_save_cal,2);
    idx_model_cal = min(170, nsave_cal);
    model_dx_cal = - ( x_s_save_cal(:,idx_model_cal) - x_s_save_cal(:,1) ); % convert to BRIE convention

    % Build model table and join with observations
    BRIE_init_cal = readtable(Initial_shoreline); BRIE_init_cal.id = string(BRIE_init_cal.id);
    Tmodel_cal = table(BRIE_init_cal.id, model_dx_cal, 'VariableNames', {'id','dx_model'});
    Tcmp = innerjoin(Tobs(:,{'id','dx'}), Tmodel_cal, 'Keys','id');

    % Basic stats and RMSE
    obs_mean = mean(Tcmp.dx,'omitnan'); obs_std = std(Tcmp.dx,'omitnan'); obs_med = median(Tcmp.dx,'omitnan');
    mod_mean = mean(Tcmp.dx_model,'omitnan'); mod_std = std(Tcmp.dx_model,'omitnan'); mod_med = median(Tcmp.dx_model,'omitnan');
    obs_iqr = iqr(Tcmp.dx); mod_iqr = iqr(Tcmp.dx_model);
    residuals = Tcmp.dx - Tcmp.dx_model; rmse = sqrt(mean(residuals.^2,'omitnan'));
    %Wasserstein distance |CDF_obs - CDF_model|
    % 1D Wasserstein = integral of |CDF_obs - CDF_model|
    sorted_obs = sort(Tcmp.dx);
    sorted_model = sort(Tcmp.dx_model);
    wass_dist = mean(abs(sorted_obs - sorted_model));

    % Kolmogorov-Smirnov test between observed and modelled distributions
    % Use kstest2 with the two samples (returns h, p, ksstat)
    [~, ~, ks_stat] = kstest2(Tcmp.dx, Tcmp.dx_model);

    % Saving the metrics for calibration 
   
    b_out.rmse = rmse; %root mean square error

    b_out.obs_iqr = obs_iqr; % IQR of the observed changes, later used for normalization

    b_out.Nrmse = rmse/obs_iqr;
    b_out.Nmean_diff = (obs_mean - mod_mean)/obs_iqr;
    b_out.Nmed_diff = (obs_med - mod_med)/obs_iqr;
    b_out.std_diff = obs_std - mod_std; 
    b_out.ks_stat = ks_stat;
    b_out.Nwasserstein = wass_dist/obs_iqr;

    b_out.drowned = drowned; %save if the barrier has drowned
end
